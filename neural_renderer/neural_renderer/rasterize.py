...
class RasterizeFunction(Function):
    ...
    def forward(ctx, faces, textures, image_size, near, far, eps, background_color,
        ...
        rgb_r, alpha_r, depth_r, face_index_map_r, weight_map_r = torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([]), torch.tensor([])
        if ctx.return_rgb:
            rgb_r = rgb_map
        if ctx.return_alpha:
        ...
        face_index_map_r = face_index_map.clone()
        weight_map_r = weight_map.clone()
        return rgb_r, alpha_r, depth_r, face_index_map_r, weight_map_r

    @staticmethod
    def backward(ctx, grad_rgb_map, grad_alpha_map, grad_depth_map, grad_face_index_map, grad_weight_map):
        '''
        Backward pass
        '''
...
def rasterize_rgbad(
    ...
    face_index_map = face_index_map[:, list(reversed(range(depth.shape[1]))), :]
    weight_map = weight_map[:, list(reversed(range(depth.shape[1]))), :, :]
