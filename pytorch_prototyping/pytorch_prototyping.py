...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if highway_mode != 'concat' and highway_mode != 'residual' and highway_mode != 'no_highway':
            raise ValueError('Unrecognized option for highway_mode')

        if gcn:
            self.fuse = DownBlock(inner_nc + out_channels_gcn, inner_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm,stride=1,kernal_size=3)
            # self.fuse1 = DownBlock(320, inner_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm,stride=1,kernal_size=3)
        if flag_outer:
            self.down = DownBlock(outer_nc, inner_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm)
            self.up = UpBlock(2*inner_nc, outer_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm,
                                upsampling_mode=upsampling_mode)
        else:
            self.down = DownBlock(outer_nc, inner_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm)
            self.up = UpBlock(inner_nc, outer_nc, use_dropout=use_dropout, dropout_prob=dropout_prob, norm=norm,
                                upsampling_mode=upsampling_mode)

    def forward(self, x, v_fea = None):
        if self.gcn:
            y = self.down(x)
            v_fea = v_fea.unsqueeze(-1).unsqueeze(-1).repeat_interleave(y.shape[2], dim = 2).repeat_interleave(y.shape[3], dim = 3)
            y = torch.cat((y, v_fea), dim = 1)
            y = self.fuse(y)
            # y = self.fuse1(y)
            y = self.submodule(y)
            y = self.up(y)
        ...
        if self.highway_mode == 'no_highway':
            return y
        elif self.highway_mode == 'residual':
            return x + y
        elif self.highway_mode == 'concat':
...
class Unet(nn.Module):
    ...
    def __init__(self,
        ...
        self.outermost_highway_mode = outermost_highway_mode
        ...
        '''self.in_layer += [Conv2dSame(96, nf0, kernel_size=3, bias=True if norm is None else False)]
        if norm is not None:
            self.in_layer += [norm(nf0, affine=True)]
        self.in_layer += [nn.LeakyReLU(0.2, True)]'''
        self.in_layer = nn.Sequential(*self.in_layer)
        ...
        self.unet_block = UnetSkipConnectionBlock(min(2 ** (num_down-1) * nf0, max_channels),
                                                  ...
                                                  upsampling_mode=upsampling_mode, flag_outer = False)

        for i in list(range(1, num_down - 1))[::-1]:
            self.unet_block = UnetSkipConnectionBlock(min(2 ** i * nf0, max_channels),
                                                      ...
                                                      norm=norm,
                                                      upsampling_mode=upsampling_mode)
        # outermost unet block
        self.unet_block = UnetSkipConnectionBlock(min(1 * nf0, max_channels),
                                                    min(2 ** (0 + 1) * nf0, max_channels),
                                                    use_dropout=use_dropout,
                                                    dropout_prob=dropout_prob,
                                                    submodule=self.unet_block,
                                                    norm=norm,
                                                    upsampling_mode=upsampling_mode, gcn = self.use_gcn, out_channels_gcn = out_channels_gcn, highway_mode = self.outermost_highway_mode)

        # Define the out layer. Each unet block concatenates its inputs with its outputs - so the output layer
        # automatically receives the output of the in_layer and the output of the last unet layer.
        if self.outermost_highway_mode == 'no_highway' or self.outermost_highway_mode == 'residual':
            self.out_layer = [Conv2dSame(nf0,
                                        out_channels,
                                        kernel_size=3,
