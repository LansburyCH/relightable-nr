...
class Renderer(nn.Module):
    def __init__(self, image_size=256, anti_aliasing=True, background_color=[0,0,0],
        ...
        if self.camera_mode == 'projection':
            ...
            self.scale = scale
            # if offset is None:
            #     self.offset = torch.cuda.FloatTensor([[0., 0.]])
            # if scale is None:
            #     self.scale = torch.cuda.FloatTensor([[1., 1.]])
    ...
    def forward(self, vertices, faces, textures=None, mode=None, K=None, R=None, t=None, dist_coeffs=None, orig_size=None, offset = None, scale = None):
        '''
        ...
        if mode is None:
            return self.render(vertices, faces, textures, K, R, t, dist_coeffs, orig_size, offset = offset, scale = scale)
        elif mode is 'rgb':
    ...
    def render(self, vertices, faces, textures, K=None, R=None, t=None, dist_coeffs=None, orig_size=None, offset = None, scale = None):
        # fill back
        if self.fill_back:
            faces = torch.cat((faces, faces[:, :, list(reversed(range(faces.shape[-1])))]), dim=1).detach()
            textures = torch.cat((textures, textures.permute((0, 1, 4, 3, 2, 5))), dim=1)
        ...
        if self.camera_mode == 'look_at':
            ...
            if scale is None:
                scale = self.scale
            vertices = nr.projection(vertices, K, R, t, dist_coeffs, orig_size, offset = offset, scale = scale)

        # rasterization
        faces_v = nr.vertices_to_faces(vertices, faces)
        out = nr.rasterize_rgbad(
            faces_v, textures, self.image_size, self.anti_aliasing, self.near, self.far, self.rasterizer_eps,
            self.background_color)
        return out['rgb'], out['depth'], out['alpha'], out['face_index_map'], out['weight_map'], vertices, faces_v, faces
